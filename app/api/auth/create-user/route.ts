...
export async function POST(request: Request) {
  try {
    ...
    // Set user as active and verified immediately (no email verification required)
    const userDataWithDefaults = {
      ...userData,
      password_hash: placeholderHash,
      status: "active", // Set to active immediately
      email_verified: true, // Set to verified immediately
    }

    // Insert the new user
    const { error: insertError } = await supabase.from("users").insert(userDataWithDefaults)
